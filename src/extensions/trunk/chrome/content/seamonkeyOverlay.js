...
IeTab.prototype.ietabContextMenuPopup = function(e) {
	...
	var item_disabled = (gIeTab.getStrPref("coral.ietab.mode","")!="advanced") || !gIeTab.getBoolPref("coral.ietab.cookieSync");
...
IeTab.prototype.updateStatusIcon = function() {
   ...
   if (img) {
      img.setAttribute("engine", (gIeTab.isIeEngine()?"eng_ie":"eng_sm"));
...
IeTab.prototype.updateBackForwardButtons = function() {
   try {
      ...
      gIeTab.updateObjectDisabledStatus("back-button", canBack);
      gIeTab.updateObjectDisabledStatus("forward-button", canForward);
   }catch(e){}
}
...
IeTab.prototype.updateStopReloadButtons = function() {
   try {
      ...
      gIeTab.updateObjectDisabledStatus("reload-button", ietab ? ietab.canRefresh : !isBlank);
      gIeTab.updateObjectDisabledStatus("stop-button", ietab ? ietab.canStop : isLoading);
   }catch(e){}
}
...
IeTab.prototype.updateEditMenuItems = function(e) {
   ...
   var ietab = gIeTab.getIeTabElmt();
   if (ietab) {
      gIeTab.updateObjectDisabledStatus("menu_cut", ietab.canCut);
      gIeTab.updateObjectDisabledStatus("menu_copy", ietab.canCopy);
      gIeTab.updateObjectDisabledStatus("menu_paste", ietab.canPaste);
   }
}
...
IeTab.prototype.viewPage = function(e, flags) {
   ...
   gIeTab.switchTabEngine(aTab, gIeTab.getBoolPref("coral.ietab.alwaysNewTab", false), flags);
}
...
IeTab.prototype.statusbarContextMenuPopup = function() {
	var b = (gIeTab.getIeTabElmt() != null) || (gIeTab.getStrPref("coral.ietab.mode","")!="advanced") || !gIeTab.getBoolPref("coral.ietab.cookieSync");
	var showicon = gIeTab.getBoolPref("coral.ietab.icon.pagelink", false);	
	var menuitem = null;
...
IeTab.prototype.updateContextMenu = function() {
	var extAppName = gIeTab.getExtAppName(gIeTab.getStrPref("coral.ietab.extAppPath", ""));
	if ( extAppName != "" ) {
...
IeTab.prototype.checkWhatsNew = function() {
	...
	if (kExtensionManagerCID in Components.classes) {
		...
		if (extmgr) {
			...
			if (ietab) {
				var oldver = gIeTab.getStrPref("coral.ietab.version", "");
				if (ietab.version > oldver) {
					gIeTab.setStrPref("coral.ietab.version", ietab.version);
		...
		AddonManager.getAddonByID("coralietab@mozdev.org", function(aAddon) {
			...
			if (aAddon.version > oldver) {
				gIeTab.setStrPref("coral.ietab.version", aAddon.version);
...
IeTab.prototype.installToolbarButton = function() {
	...
	if (this.unhideToolbar && this.unhideToolbar())
	{
		...
		gIeTab.setBoolPref("coral.ietab.statusbar", false);
...
IeTab.prototype.toolbarInsertBefore = function() {
	return document.getElementById('bookmarks-button');
}
